...
// Try to import NativeBiometric, but handle if it's not available
let NativeBiometric = null;
if (Capacitor.isNativePlatform()) {
  try {
    NativeBiometric = require('capacitor-native-biometric').NativeBiometric;
  } catch (e) {
...
export const useVault = () => {
  ...
  const cacheDecryptedThumbnail = useStore((state) => state.cacheDecryptedThumbnail);
   ...
   */
  const setupVaultWithPassword = useCallback(async (password, settings = {}) => {
    try {
   ...
   */
  const uploadToVault = useCallback(async (files) => {
    ...
    try {
      ...
      for (const file of files) {
        // Encrypt file
        const { blob: encryptedBlob, metadata: encryptionMetadata } = await encryptFile(file, password);

        // Generate unique ID
        const photoId = `${user.uid}_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
      ...
      showNotification(t('vault:notifications.uploadSuccess', { count: files.length }), 'success');
    } catch (error) {
  ...
  }, [user, isVaultUnlocked, addPhotoToVault, setVaultLoading, showNotification, lockVaultSafely, t]);

  /**
   * Delete photo from vault
   */
  const deleteFromVault = useCallback(async (photoId) => {
  ...
  }, [user, isVaultUnlocked, vaultPhotos, removePhotoFromVault, showNotification, t]);

  /**
   * Decrypt and get photo blob URL
   */
  const getDecryptedPhotoUrl = useCallback(async (photo) => {
  ...
  }, [isVaultUnlocked, getCachedThumbnail, cacheDecryptedThumbnail, lockVaultSafely]);
  ...
  }, [isVaultUnlocked, vaultSettings.autoLockTimeout, checkAutoLock, lockVaultSafely]);
