...
function AppContent() {
  ...
  const handleAlbumSave = async (albumData) => {
    try {
      if (editingAlbum) {
        ...
        // Album already created by UploadModal
  ...
  const handleCreateAlbumFromUpload = async (albumData) => {
    try {
      // Album already created by UploadModal, only refresh
      await refreshData()
      setNotification({ message: 'Album opprettet', type: 'success' })
      return albumData.id // use existing id
    } catch (err) {
      console.error('Album creation error:', err)
      setNotification({
        message: 'Feil ved opprettelse av album',
        type: 'error',
      })
      throw err
    }
    ...
    <div className="min-h-screen relative">
      ...
      <main className="relative z-10">
        ...
        {currentPage === 'admin' && isAdmin && (
          <AdminDashboard onBack={() => setCurrentPage('more')} />
        )}
      </main>
